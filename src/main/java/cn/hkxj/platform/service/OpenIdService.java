...
// =============================================================上面是原先的代码，下面的改过的代码
        if (isPlus(appid)) {
            return wechatOpenidMapper.isOpenidBind(openid)==1;//自己新添的代码,已测试
        }
        return wechatOpenidMapper.isOpenidBind(openid) == 1;//自己新添的代码，已测试
    }
...
//=======================================下面是改过的代码替代上面的源码，若要恢复记得把方法返回的集合泛型换成Openid
  ...
  //====================================================上面是源码，下面是修改过的代码
        ...
        if( wechatOpenidProperties.getAppId().equals(appid))  {
            List<WechatOpenid> openidList = wechatOpenIdDao.selectByPojo(new WechatOpenid().setOpenid(openid));//后加代码
            if (openidList.size() == 1) {
                return studentDao.selectStudentByAccount(openidList.get(0).getAccount());
            }
            return null;
        }
        List<WechatOpenid> openidList = getOpenid(openid, appid);//后添加的代码
        if (openidList.size() == 0) {
            throw new IllegalArgumentException("user not bind openid: " + openid + " appid: " + appid);
        }
        Integer account = openidList.get(0).getAccount();
 ...
 //==========================================================上面是源代码，下面是改过的代码
        if (isPlus(appid)) {
            wechatOpenidMapper.openidUnbind(openid);//自己新加的方法代替上一条代码
        } else {
            wechatOpenidMapper.openidUnbind(openid);//自己新加的方法代替上一条代码
        }
 ...
 //============================================================上面是源代码，下面是改过的代码
  ...
  //===============================================上面是源代码，下面是改过的代码
        if (isPlus(appid)) {
            return wechatOpenidMapper.getAllOpenidsFromOneClass(classId, openid);//新添加的代码，替换原先的测试代码
        } else {
            return wechatOpenidMapper.getAllOpenidsFromOneClass(classId, openid);//新添加的代码，替换原先的测试代码
        }
